...
//        getContentPane().setBackground(Color.WHITE);
        ...
        setLayout(null);
        setVisible(true);
        JLabel lab = new JLabel("Apply Leave (Student)");
        lab.setBounds(150, 40, 330, 30);
        lab.setFont(new Font("serif", Font.BOLD, 26));
        add(lab);
        JLabel labcap = new JLabel("Search By Roll #");
        ...
        try {
            Conn con = new Conn();
            ResultSet rs = con.sta.executeQuery("SELECT * FROM `student_record`");
            while (rs.next()) {
                ops.add(rs.getString("roll_number"));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == btns) {
            String rollno = ops.getSelectedItem();
            String duration = durationle.getSelectedItem();
            String date = ((JTextField) leavedt.getDateEditor().getUiComponent()).getText();

            try {
                Conn con = new Conn();
                String query = "INSERT INTO `student_leave`(`roll_no`,`date`,`duration`)" + " VALUES ('" + rollno + "','" + duration + "','" + date + "')";
                con.sta.executeUpdate(query);
                JOptionPane.showMessageDialog(null, "Great! Student Leave Added Successfully. ");
            }catch(Exception e){
                e.printStackTrace();
            }
