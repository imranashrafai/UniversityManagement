...
public class TeacherLeaveDetails extends JFrame implements ActionListener {
    ...
    TeacherLeaveDetails() {
        setSize(900, 700);
        setLocation(250, 18);
        setTitle("View Teachers' Leave Details");
        ...
        JLabel tabletop = new JLabel("Search By Emplpyee ID");

        tabletop.setBounds(50, 43, 180, 30);
        tabletop.setFont(new Font("serif", Font.BOLD, 17));
        add(tabletop);
        crollnum = new Choice();
        crollnum.setBounds(280, 50, 130, 30);
        ...
        try {
            Conn c = new Conn();
            ResultSet rs = c.sta.executeQuery("SELECT * FROM `teacher_leave`");
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == search) {
            String query = "SELECT * FROM `teacher_leave` WHERE emp_id = '" + crollnum.getSelectedItem() + "'";
            try {
                Conn c = new Conn();
